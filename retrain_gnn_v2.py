...
Retrains GraphSAGE on the COMBINED pool (baseline + structuring), using the
same architecture and train/test split methodology as the original
train_gnn.py. Saves the new model as gnn_model_v2.pt (v1 is untouched, so
you can always compare before/after).

This is the "self-healing" half of the story: drift was detected,
retraining was recommended, and this is that retrain actually happening.
"""
import pandas as pd
import numpy as np
import torch
...
TRANSACTIONS_FILE = "combined_transactions.csv"
FEATURES_FILE = "combined_account_features.csv"
...
def main():
    ...
    print("\n=== GraphSAGE v2 (retrained on combined pool) -- held-out test results ===")
